...
import { View, ViewStyle, FlatList, TextStyle, RefreshControl } from "react-native"
...
export const ProcedimentosScreen: FC<ProcedimentosProps> = observer(function ProcedimentosScreen() {
  ...
  useEffect(() => {
    ...
    examSchedulingStore.setExamType("PP")
    // Fetch image exams (EI = Exames de Imagem)
    fetchProcedimentosEcommerce({ tipo_procedimento: "PP" })
        ...
        <View style={themed($examListContainer)}>
          {error ? (
            <EmptyState
              ...
              content={error}
              buttonOnPress={() => fetchProcedimentosEcommerce({ tipo_procedimento: "PP" })}
            ...
            <FlatList
              ...
              refreshControl={
                <RefreshControl
                  refreshing={loading}
                  onRefresh={() => fetchProcedimentosEcommerce({ tipo_procedimento: "PP" })}
                  colors={["#1E90FF"]}
                  tintColor="#1E90FF"
                />
              }
              ListEmptyComponent={
                !loading ? (
                  <EmptyState
                    heading="Nenhum procedimento encontrado"
                    content="Não foram encontrados procedimentos disponíveis."
                    buttonOnPress={() => fetchProcedimentosEcommerce({ tipo_procedimento: "PP" })}
                    button="Recarregar"
                  />
                ) : null
